...
async function extractYouTubeTranscriptFromPage(preferredLanguage) {
  ...
  async function readTranscriptFromCaptionTracks(language) {
    ...
    for (const format of formats) {
      const response = await fetch(buildCaptionUrl(track.baseUrl, format), {
        credentials: "include"
      });
      ...
      const transcript = parseCaptionResponse(await response.text(), format);
      if (transcript.trim()) {
        return {
          ok: true,
          transcript,
    ...
    return {
      ok: false,
      error: lastStatus
        ? `Could not fetch YouTube captions (${lastStatus}).`
        : "YouTube returned caption tracks, but none contained readable text."
    };
  }
